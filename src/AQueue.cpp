...
T AQueue<T>::front() const
{
  ...
  {
    ostringstream out;
    out << "Error: <Aqueue>::front() attempt to get item"
...
/** @brief enqueue value on end of queue
 *
 * Enqueue the indicated value to the back of this Queue
 * of values.  This method will grow the queue allocation if
 * needed to accomodate appending the new value. This operation
 * is constant time O(1) if the array/buffer does not need to grow
 * its allocation, otherwise it is O(n).
 *
 * @param value The value to enqueue on front of the current queue.
 */
template<class T>
void AQueue<T>::enqueue(const T& value)
{
  ...
  // queue allocation is guaranteed to have some room now, so
  // simply append the value to the end of the current values
  // by incrementing back index by 1 (being carefult to wrap
  // back around the buffer) and incrementing the size as well.
  backIndex = (backIndex + 1) % allocationSize;
  values[backIndex] = value;
  this->size += 1;
}

/** @brief dequeue the front value
 *
 * Dequeue the item from the front of the queue.  This is a
 * constant time  O(1) operation for an array based implementation.
 */
template<class T>
void AQueue<T>::dequeue()
{
  ...
  // removing front item of an array means the front index goes up to next
  // position (being careful to wrap it around the circular buffer if
  // needed), and our queue size reduces by 1
